      ...
      <>
        ...
        <div className="wrapper">
          {props.ArtistData.map((artist) => (
            <Card
              img={
                artist.images.length == 0 ? NoimageLink : artist.images[0].url
              }
              title={artist.name}
...
function Card(props) {
  return (
    <div
      className="card"
      onClick={() => {
        props.function(props.id, 0, props.title);
      }}
    >
      <div className="card__body">
        ...
        <p className="card__description">
          {props.followers.toLocaleString("en-US")} followers
        </p>
      </div>
      <br></br> <br></br>
      <div className="rating">{stars(props.popularity)}</div>
      <br></br>
    </div>
  );
}
